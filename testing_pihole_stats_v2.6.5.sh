...
generate_report() {
    ...
    # FIX v2.7.3: Fallback to defaults if Config file provided 0 valid tiers
    # This prevents the "syntax error near ','" caused by an empty SQL columns list.
    if [ ${#sorted_limits[@]} -eq 0 ]; then
        if [ "$SILENT_MODE" = false ]; then echo "⚠️ Warning: No valid tiers found in config. Using defaults." >&2; fi
