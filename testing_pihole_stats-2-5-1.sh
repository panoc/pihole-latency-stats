...
generate_report() {
    ...
    for i in "${!labels[@]}"; do
        # STRICT SQL FIX: Switched from "Double Quotes" to 'Single Quotes' for string literals
    ...
    # STRICT SQL FIX: All SELECT statements now use 'single quotes' for text.
    TEXT_REPORT_SQL="
        SELECT '=========================================================';
...
# --- 10. AUTO-DELETE OLD LOGS ---
if [ -n "$SAVE_DIR" ] && [ -d "$SAVE_DIR" ] && [ -n "$MAX_LOG_AGE" ] && [[ "$MAX_